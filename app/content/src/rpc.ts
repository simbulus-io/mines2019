...
const blob_cache = new BlobCache({url: `${API_BASE_URL}/job/cache`});
...
const rpc = async (job:any): Promise<any | null>  => {
  ...
  try {
    const hresp = await fetch(`${API_BASE_URL}/job/schedule`,{
      method: 'POST',
      headers: {
    ...
    const polling_url      = `${API_BASE_URL}/job/results?job_id=${job_id}`
